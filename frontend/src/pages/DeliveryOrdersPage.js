...
export default function DeliveryOrdersPage() {
  ...
  // Auto-compress: large photos rarely need full 4K resolution for a
  // delivery slip thumbnail. Resize to max 1600 px and re-encode as JPEG
  // ~80% so most uploads land under Firestore's 700 KB inline ceiling.
  const compressImage = (file) => new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        // Try progressively lower quality until result is < 700 KB.
        const targets = [0.85, 0.75, 0.65, 0.55, 0.45];
        for (const q of targets) {
          const dataUrl = canvas.toDataURL("image/jpeg", q);
          if (dataUrl.length <= 700_000) return resolve(dataUrl);
        }
        // Big file fallback — return at lowest quality. Backend will push
        // it to Firebase Storage automatically.
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      setForm(f => ({ ...f, image_data_url: dataUrl }));
    } catch {
