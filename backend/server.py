...
async def startup():
    ...
    # had been previously reduced by supplier returns. We never overwrite
    # historical purchase values; net payable is computed from credit notes.
    async for pur in db.purchases.find({"supplier_return_adjustments": {"$exists": True, "$ne": []}}):
